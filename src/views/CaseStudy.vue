<template>
	...
	<section class="section section__single-resource">
		...
		<app-rec-links class="rec-link" v-if="post.meta_box.hasOwnProperty('_casestudy_links') && post.meta_box._casestudy_links.length > 0" :rec-links="post.meta_box._casestudy_links">
		</app-rec-links>
...
<script>
	...
	export default {
		...
		created() {
			...
			axios.get('https://parkpeople.ca/wp-json/wp/v2/case-study/' + this.id + '?_embed')
			...
			.then(response => {
				...
				if (this.post.wpml_translations.length >0) {
					...
					axios.get('https://parkpeople.ca/wp-json/wp/v2/case-study/' + translatedID + '?_embed') 
					.then(response => {
