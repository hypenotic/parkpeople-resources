<template>
  <div v-if="post != null" class="resource-body">
    ...
    <section style="margin-top: -160px;">
      <div class="columns">
        <div
          ...
          style="background-color: white; padding: 3rem;position:relative;"
        >
          ...
          <div class="columns" style="margin-top: 32px;">
            <div class="column is-6">
              <div
                v-if="post.meta_box._research_link_title != ''"
                class="research__thumbnail"
              >
                <img
                  :src="post.meta_box._research_link_thumbnail[0].full_url"
                  :alt="post.meta_box._research_link_title"
                />
              </div>
            ...
            <div class="column is-6">
              <div v-if="post.meta_box._research_link_title != ''">
                ...
                <p>
                  <span v-html="post.meta_box._research_link_subtitle"></span> |
                  <span
                    v-if="
                      post.meta_box.hasOwnProperty('_research_link_date') &&
                        post.meta_box._research_link_date != ''
                    "
                    v-html="post.meta_box._research_link_date"
                  ></span>
                </p>
                <div
                  class="research__excerpt"
                  v-html="post.excerpt.rendered"
                  style="margin: 0;"
                ></div>
                <a
                  v-if="lang == 'fr'"
                  :href="post.meta_box._research_link_button"
                  class="button button--rounded button--orange"
                  target="_blank"
                  ><i class="fa fa-external-link" aria-hidden="true"></i>Lis</a
                >
                <a
                  v-else
                  :href="post.meta_box._research_link_button"
              ...
              <div v-else>
                ...
                <p>
                  <span v-html="post.meta_box._research_pdf_subtitle"></span> |
                  <span
                    v-if="
                      post.meta_box.hasOwnProperty('_research_pdf_date') &&
                        post.meta_box._research_pdf_date != ''
                    "
                    v-html="post.meta_box._research_pdf_date"
                  ></span>
                </p>
                <div
                  class="research__excerpt"
                ...
                <a
                  v-if="lang == 'fr'"
                  :href="post.meta_box._research_pdf_file[0].url"
                  class="button button--rounded button--orange"
                  target="_blank"
                  ><i class="fa fa-download" aria-hidden="true"></i>Cliquez pour
                  télécharger</a
                >
                <a
                  v-else
                  ...
                  class="button button--rounded button--orange"
                  target="_blank"
                  ><i class="fa fa-download" aria-hidden="true"></i> Click to
                  download</a
                >
              </div>
            </div>
    ...
    <div
      class="quote"
      v-if="
        post.meta_box._research_content_quote != undefined &&
          post.meta_box._research_content_quote != ''
      "
    >
      <blockquote
        class="full-width-quote"
        v-html="post.meta_box._research_content_quote"
      ></blockquote>
      <cite>
    ...
    <section
      class="section research__summary"
    ...
    <app-rec-links
      class="rec-link"
      v-if="
        post.meta_box.hasOwnProperty('_research_links') &&
          post.meta_box._research_links.length > 0
      "
      :recLinks="post.meta_box._research_links"
    >
    </app-rec-links>

    <div
      v-if="post.pure_taxonomies.activity && lang == 'fr'"
      class="research__activity-list"
    >
      <b>Faire dans les parcs: </b>
      <ul>
        <li v-for="tax in post.all_lang_taxonomies.activity" :key="tax.name">
          {{ tax.activity_french[0] | toUppercase }}
        </li>
      </ul>
    </div>
    <div
      v-if="post.pure_taxonomies.learn && lang == 'fr'"
      class="research__learn-list"
    >
      <b>Savoir les parcs: </b>
      <ul>
        <li v-for="tax in post.all_lang_taxonomies.learn" :key="tax.name">
          {{ tax.learn_french[0] | toUppercase }}
        </li>
      </ul>
    </div>
    <div
      v-if="post.pure_taxonomies.activity && lang == 'en'"
      class="research__activity-list"
    >
      <b>Do in parks: </b>
      <ul>
        <li v-for="tax in post.pure_taxonomies.activity" :key="tax.name">
          {{ tax.name | toUppercase }}
        </li>
      </ul>
    </div>
    <div
      v-if="post.pure_taxonomies.learn && lang == 'en'"
      class="research__learn-list"
    >
      <b>Know about parks: </b>
      <ul>
        <li v-for="tax in post.pure_taxonomies.learn" :key="tax.name">
          {{ tax.name | toUppercase }}
        </li>
      </ul>
    </div>
...
export default {
  ...
  created() {
    ...
    axios
      .get(
        "https://parkpeople.ca/wp-json/wp/v2/research/" + this.id + "?_embed"
      )
      ...
      .then(response => {
        ...
        if (this.post.wpml_translations.length > 0) {
          ...
          axios
            .get(
              "https://parkpeople.ca/wp-json/wp/v2/research/" +
                translatedID +
                "?_embed"
            )
            .then(response => {
